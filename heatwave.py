...
def apply_dbscan_clustering(data_scaled):
    ...
    Returns:
        np.ndarray: Cluster labels from DBSCAN.
    """
    # Apply DBSCAN clustering to the selected features
    dbscan = DBSCAN(eps=0.5, min_samples=10)  # Define the DBSCAN model
    labels = dbscan.fit_predict(data_scaled)  # Fit the model and get the cluster labels
